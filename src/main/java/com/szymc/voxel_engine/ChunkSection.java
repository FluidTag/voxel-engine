...
public class ChunkSection {
	...
	private byte getWorldLookup(int cx, int cy, int cz) {
		if (cx >= 0 && cx <= 15 && cy >= 0 && cy <= 15 && cz >= 0 && cz <= 15) {
			return this.chunk[cx][cy][cz];
		}
		
		return worldReference.getBlockAtWorldPos(cx, cy, cz);
	}
	
	private final BitSet meshed = new BitSet(16*16);
	private final int[] point = new int[3];
	private final int[] c1 = new int[3];
	private final int[] c2 = new int[3];
	private final int[] c3 = new int[3];
	private final int[] c4 = new int[3];
				...
				for (point[axis] = 0; point[axis] < 16; point[axis]++) { // Depth
					...
					for (point[v] = 0; point[v] < 16; point[v]++) { // Vertical
						for (point[u] = 0; point[u] < 16; point[u]++) { // Horizontal
							...
							if (meshed.get(point[v]*16 + point[u])) continue;
							...
							int absNy = worldY + ny;
							int absNz = worldZ + nz;
							if (getWorldLookup(absNx, absNy, absNz) != 0) continue;
							...
							while (uBound < 16) {
								...
								if (meshed.get(point[v]*16 + point[u])) break;
								if (chunk[point[0]][point[1]][point[2]] != currentBlock) break;
								if (getWorldLookup(abSubX, abSubY, abSubZ) != 0) {
									break;
								}
							...
							while (vBound < 16) {
								...
								for (int uCheck = uStart; uCheck < uBound; uCheck++) {
									...
									int abSubY = worldY + point[1] + (axis == 1 ? direction : 0);
									int abSubZ = worldZ + point[2] + (axis == 2 ? direction : 0);
									if (meshed.get(point[v]*16 + point[u])) {
										passed = false;
										break;
									}
